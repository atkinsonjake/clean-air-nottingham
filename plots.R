...
# Data
comparisons <- read.csv('/Users/jakeatkinson/Documents/Personal/clean-air-nottingham/response_data/24102023/processed_data_can_sensors_hourly_calibrated.csv')
comparisons <- na.omit(comparisons)
comparisons$sensor_index <- factor(comparisons$sensor_index)

comparisons <- comparisons[comparisons$fields %in% c("pm1.0_atm", "pm2.5_atm", "pm10.0_atm", "pm2.5_calibrated"), ]
...
CAN_COLOURS <-  c("#3D5A80", "#98C1DA", "#EE6C4D",  "#f4a261",  "#2a9d8f")


grouped_df <- aggregate(reading ~ sensor_index + fields, data = comparisons, FUN = mean)
ggplot(data = grouped_df, aes(x = sensor_index, y = reading, fill = fields)) +
...
# (2022-12-03 - 2023-06-01)

avg_data <- comparisons_no_bank_hols %>%
  group_by(day_of_week, fields) %>%
  summarize(avg_reading = mean(reading))
ggplot(avg_data, aes(x = day_of_week, y = avg_reading, group = fields, color = fields)) +
  geom_line() +
  ...
  theme_minimal() +
  theme(panel.grid = element_blank())
# (2022-12-03 - 2023-06-01, exlc bank holidays)

avg_data <- comparisons %>%
  group_by(time, fields) %>%
  summarize(avg_reading = mean(reading))
avg_data$hour <- format(strptime(avg_data$time, "%H:%M:%S"), "%H")
ggplot(avg_data, aes(x = hour, y = avg_reading, group = fields, color = fields)) +
...
ggplot(grouped_df, aes(x = fields, y = reading, fill = is_bank_holiday)) +
  geom_bar(stat = "identity", position = "dodge") +
...
ggplot(grouped_df, aes(x = date_only, y = reading, group = fields, color = fields)) +
  geom_line() +
...
grouped_df$date_only <- as.Date(grouped_df$date_only)
ggplot(grouped_df, aes(x = date_only, y = reading)) +
  geom_line() +
...
pm_25_calibrated <- subset(comparisons, fields == "pm2.5_calibrated")
grouped_df <- aggregate(reading ~ date_only + fields, data = pm_25_calibrated, FUN = mean)
grouped_df$date_only <- as.Date(grouped_df$date_only)
ggplot(grouped_df, aes(x = date_only, y = reading)) +
  geom_line() +
  labs(x = "Date", y = "Mean reading (μgm-3)") +
  ggtitle("PM2.5 (calibrated), mean daily reading") +
  geom_line(color = "#98c1d9")  +
  theme_minimal() +
  theme(panel.grid = element_blank()) +
  geom_hline(yintercept = 5, color = "#ee6c4d") +
...
subset_df <- subset(subset_df, month > 3)
ggplot(subset_df, aes(x = sensor_index, y = reading)) +
  geom_boxplot() +
...
subset_df <- subset(comparisons, fields = "pm2.5_calibrated")
subset_df <- subset(subset_df, month > 3)
ggplot(subset_df, aes(x = sensor_index, y = reading)) +
  geom_boxplot() +
  labs(x = "Sensor", y = "Reading (μgm-3)") +
...
avg_data <- comparisons_no_bank_hols %>%
  group_by(day_of_week, fields) %>%
  summarize(avg_reading = mean(reading))
plot2 <- ggplot(avg_data, aes(x = day_of_week, y = avg_reading, group = fields, fill = fields)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Day", y = "Mean reading (μgm-3)", color = "Pollutant") +
  ggtitle("How does pollution change across the week?") +
  scale_fill_manual(values = CAN_COLOURS) +
...
# gpt <- read.csv('/Users/jakeatkinson/Desktop/bot-v-agent-allocation-daily.csv')
# gpt <- gpt %>%
#   mutate(date = dmy(date),
#          year = year(date))
# 
# week <- subset(gpt, day %in% c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday"))
# week_2023 <- subset(week, year(date) == 2023)
# week_2023 <- week_2023 %>%
#   mutate(week = week(date))
# 
# week_2023_summary <- week_2023 %>%
#   group_by(week) %>%
#   summarise(proportion_allocated = mean(bot))
# 
# data_for_plot <- subset(week_2023_summary, week > 12)
# ggplot(data_for_plot, aes(x = week, y = proportion_allocated)) +
#   geom_line() +
#   geom_vline(xintercept = 26, color = "green", linetype = "solid") + 
#   geom_vline(xintercept = 30, color = "red", linetype = "solid") + 
#   ggtitle('test') +
#   xlab('test') +
#   ylab('test')
